...
- **DeepSeek Harness** — zero-touch integration: a built-in monitor polls `$DSH_HOME/storages/` (`workspace.json` + `session_projcache.json`) and installs **no hooks, plugins, or config into DSH** (install from Settings → Agents or run `npm run install:dsh`). Phase 1 drives Clawd animations and the Session HUD with seconds-level latency (sampling, not event-exact); state-only — permission decisions stay in DSH's own approval-preset / sandbox flow and Clawd never pops a permission bubble. See the [DeepSeek Harness guide](docs/guides/dsh-setup.md)
