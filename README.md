...
- **DeepSeek Harness** — zero-touch perception plus an optional Clawd-managed interaction bridge: a built-in monitor polls `$DSH_HOME/storages/` (`workspace.json` + `session_projcache.json`) and never writes into DSH on its own; clicking **Install** in Settings → Agents additionally registers Clawd's `@dsh-external/dsh-clawd-bridge` plugin into the DSH web profile (via the official `dsh plugin` command, idempotent), which routes DSH's `ask_user_question` and sandbox-approval requests to Clawd's permission bubbles and returns your decision to DSH (startup sync stays read-only; uninstall removes the bridge). Seconds-level latency (sampling, not event-exact). See the [DeepSeek Harness guide](docs/guides/dsh-setup.md)
