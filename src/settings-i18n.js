...
(function initSettingsI18n(root) {
  const STRINGS = {
    en: {
      ...
      discordPresenceMirrorAnimationDesc: "Use the supported Clawd state animation currently shown by the pet as your status image. More detailed than the standard status — some animations reveal how many sessions are running. Unsupported animations and other themes use state-based Clawd sprites.",
    ...
    zh: {
      ...
      discordPresenceMirrorAnimationDesc: "将宠物当前显示的受支持 Clawd 状态动画用作状态图片。这比标准状态更详细——部分动画会透露正在运行的会话数量。不支持的动画和其他主题会使用按状态推导的 Clawd 贴图。",
    ...
    "zh-TW": {
      ...
      discordPresenceMirrorAnimationDesc: "將寵物目前顯示的受支援 Clawd 狀態動畫用作狀態圖片。這比標準狀態更詳細——部分動畫會透露正在執行的工作階段數量。不支援的動畫和其他主題會使用依狀態推導的 Clawd 貼圖。",
    ...
    ko: {
      ...
      discordPresenceMirrorAnimationDesc: "펫에 현재 표시되는 지원 대상 Clawd 상태 애니메이션을 상태 이미지로 사용합니다. 표준 상태보다 더 자세하며, 일부 애니메이션은 실행 중인 세션 수를 드러낼 수 있습니다. 지원되지 않는 애니메이션과 다른 테마는 상태 기반 Clawd 스프라이트를 사용합니다.",
    ...
    ja: {
      ...
      discordPresenceMirrorAnimationDesc: "ペットに現在表示されている対応済みの Clawd 状態アニメーションをステータス画像として使用します。標準ステータスより詳細で、一部のアニメーションから実行中のセッション数がわかる場合があります。未対応のアニメーションと他のテーマでは、状態に基づく Clawd スプライトを使用します。",
