...
(function initSettingsI18n(root) {
  const STRINGS = {
    en: {
      ...
      discordPresenceMirrorAnimationDesc: "Use the pet's current animation as your status image instead of the coarse state. More detailed than the standard status — some animations reveal how many sessions are running. Clawd theme only for now; other themes keep the standard sprites.",
    ...
    zh: {
      ...
      discordPresenceMirrorAnimationDesc: "用宠物当前的动画作为状态图片，而不是粗略状态。比标准状态更详细——部分动画会透露正在运行的会话数量。目前仅支持 Clawd 主题；其他主题继续使用标准贴图。",
    ...
    "zh-TW": {
      ...
      discordPresenceMirrorAnimationDesc: "用寵物目前的動畫作為狀態圖片，而不是粗略狀態。比標準狀態更詳細——部分動畫會透露正在執行的工作階段數量。目前僅支援 Clawd 主題；其他主題繼續使用標準貼圖。",
    ...
    ko: {
      ...
      discordPresenceMirrorAnimationDesc: "대략적인 상태 대신 펫의 현재 애니메이션을 상태 이미지로 사용합니다. 표준 상태보다 더 자세하며, 일부 애니메이션은 실행 중인 세션 수를 드러낼 수 있습니다. 현재는 Clawd 테마만 지원하며, 다른 테마는 표준 스프라이트를 유지합니다.",
    ...
    ja: {
      ...
      discordPresenceMirrorAnimationDesc: "大まかな状態の代わりに、ペットの現在のアニメーションをステータス画像として使用します。標準ステータスより詳細で、一部のアニメーションから実行中のセッション数がわかる場合があります。現在は Clawd テーマのみ対応。他のテーマは標準スプライトを使用します。",
