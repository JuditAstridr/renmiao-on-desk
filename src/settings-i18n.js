...
(function initSettingsI18n(root) {
  const STRINGS = {
    en: {
      ...
      permissionAutomationAutoToolsDesc: "Recommended. Supported tool requests—including terminal commands, file writes, and deletions—are approved automatically; questions and plan reviews still wait for you. CodeBuddy keeps its native approval flow.",
      ...
      permissionAutomationAutoToolsConfirmDetail: "When enabled, supported tool requests such as terminal commands, file writes, and deletions are approved without asking each time. Questions and plan reviews still wait for you. CodeBuddy keeps its native approval flow.",
    ...
    zh: {
      ...
      permissionAutomationAutoToolsDesc: "推荐档。受支持的工具请求（含终端命令、写文件、删除）会自动放行；提问和计划审批仍等你决定。CodeBuddy 继续使用原生审批。",
      ...
      permissionAutomationAutoToolsConfirmDetail: "开启后，受支持的终端命令、写文件和删除等工具请求会直接放行，不再逐次询问；提问和计划审批仍由你决定。CodeBuddy 仍使用原生审批流程。",
    ...
    "zh-TW": {
      ...
      permissionAutomationAutoToolsDesc: "建議檔。受支援的工具請求（包含終端命令、寫入檔案、刪除）會自動允許；提問和計畫審批仍等你決定。CodeBuddy 繼續使用原生審批。",
      ...
      permissionAutomationAutoToolsConfirmDetail: "開啟後，受支援的終端命令、寫入檔案和刪除等工具請求會直接允許，不再逐次詢問；提問和計畫審批仍由你決定。CodeBuddy 仍使用原生審批流程。",
    ...
    ko: {
      ...
      permissionAutomationAutoToolsDesc: "권장 모드입니다. 터미널 명령, 파일 쓰기와 삭제를 포함한 지원 도구 요청은 자동 승인되며, 질문과 계획 승인은 사용자의 결정을 기다립니다. CodeBuddy는 기본 승인 흐름을 유지합니다.",
      ...
      permissionAutomationAutoToolsConfirmDetail: "켜면 지원되는 터미널 명령, 파일 쓰기와 삭제 같은 도구 요청을 매번 묻지 않고 바로 승인합니다. 질문과 계획 승인은 계속 사용자의 결정을 기다리며, CodeBuddy는 기본 승인 흐름을 유지합니다.",
    ...
    ja: {
      ...
      permissionAutomationAutoToolsDesc: "推奨モード。ターミナルコマンド、ファイル書き込み、削除を含む対応ツールのリクエストを自動承認し、質問と計画承認はあなたの判断を待ちます。CodeBuddy は標準の承認フローを使います。",
      ...
      permissionAutomationAutoToolsConfirmDetail: "有効にすると、対応しているターミナルコマンド、ファイル書き込み、削除などのツールリクエストを毎回確認せずに承認します。質問と計画承認は引き続きあなたの判断を待ち、CodeBuddy は標準の承認フローを使います。",
