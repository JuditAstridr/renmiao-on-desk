...
// Study Companion owns the account-scoped score used by Renmi unlocks. Keep
// this read defensive: accessory rendering must fail closed to "none" if the
// study runtime is not ready or its snapshot is malformed.
function getStudyPointsTotal() {
  try {
    const snapshot = _studyRuntime && typeof _studyRuntime.getSnapshot === "function"
      ? _studyRuntime.getSnapshot()
      : null;
    const total = snapshot && snapshot.points && snapshot.points.total;
...
let holidayAccessoryRuntime = null;
...
const _settingsController = createSettingsController({
  ...
  injectedDeps: {
    getStudyPoints: getStudyPointsTotal,
...
// Floating windows must anchor to the native render rect. On Linux the
// logical pet bounds can differ from the physical window while edge pinning
// is active; using the live render rect keeps Study attached to what is
// actually visible on screen.
function getPetWindowAnchorBounds() {
  try {
    const physical = petWindowRuntime.getPhysicalRenderBounds();
...
function repositionStudyWindowNearPet(followOverride) {
  const follow = typeof followOverride === "boolean"
    ? followOverride
    : _settingsController.get("studyFollowPet") === true;
  if (!follow || !studyWindowRuntime || typeof studyWindowRuntime.repositionNearPet !== "function") {
    return false;
  }
...
let lastStudyPointsForAccessory = null;
...
const _studyWindow = createStudyWindowRuntime({
  ...
  getI18n: () => getDashboardI18nPayload(),
  shouldFollowPet: () => _settingsController.get("studyFollowPet") === true,
  getPetWindowAnchorBounds,
  getPetWindowBounds,
  getNearestWorkArea,
  getTextScale: () => getTextScaleForPetWindows(),
  iconPath: settingsWindowRuntime.getIconPath(),
...
studyIpcRuntime = registerStudyIpc({
  ...
  saveReportPoster: async (_event, payload) => {
    ...
    if (!match || match[1].length > 16 * 1024 * 1024) {
      return { status: "error", message: "invalid-report-poster" };
    }
    ...
    if (!result || result.canceled || !result.filePath) return { status: "cancel" };
    try {
      fs.writeFileSync(result.filePath, Buffer.from(match[1], "base64"));
      return { status: "ok" };
    } catch (error) {
      console.warn("Renmi: study report poster save failed:", error && error.message);
...
const _sessionHud = require("./session-hud")({
  get win() { return win; },
  get petHidden() { return petWindowRuntime.isPetHidden(); },
  // Renmi keeps agent monitoring available, but does not show the two
  // persistent pet-side status bubbles from the legacy Clawd UI. The backing
  // session/quota state and its APIs remain active for the Dashboard and
  // integrations.
  get sessionHudEnabled() { return isRenmiProfile ? false : sessionHudEnabled; },
  ...
  get sessionHudShowQuota() { return isRenmiProfile ? false : sessionHudShowQuota; },
...
const SETTINGS_MIRROR_SETTERS = {
  ...
  studyFollowPet: (v) => { repositionStudyWindowNearPet(v); },
...
holidayAccessoryRuntime = createHolidayAccessoryRuntime({
  powerMonitor,
  getSettingsSnapshot: () => _settingsController.getSnapshot(),
  getActiveTheme: () => getActiveTheme(),
  getStudyPoints: getStudyPointsTotal,
