...
- **DeepSeek Harness** — 零侵入集成：Clawd 内置的 monitor 直接轮询 `$DSH_HOME/storages/`（`workspace.json` + `session_projcache.json`），**不向 DSH 安装任何 hook / 插件 / 配置**（从 Settings → Agents 安装，或执行 `npm run install:dsh`）。Phase 1 只驱动动画与会话 HUD，秒级延迟（非事件级精确）；仅状态同步，权限决策留在 DSH 自己的 approval preset / 沙箱流程里，Clawd 不弹权限气泡。详见 [DeepSeek Harness 指南](docs/guides/dsh-setup.md)
