...
- ZCode（智谱/Z.ai，Electron 桌面 ADE）通过 `~/.zcode/cli/config.json` 做 **state-only** hook 集成。进程树：`ZCode`（主壳）→ `zcode-host-local` → `zcode-cli`（per-session agent runtime，macOS/Linux 是独立二进制，已 3.4.2 真机验证；Windows 3.3.6 走 `ZCode.exe resources/glm/zcode.cjs app-server --stdio` 复用 Electron 壳，需按 cmdline 区分，`zcode-cli.exe` 待 Windows 真机确认）。Clawd 检测工作进程 `zcode-cli`，不认桌面壳。**注意 ZCode config-file hook schema 与 Claude Code 的 `~/.claude/settings.json` 不同**：事件数组嵌在 `hooks.events.<Event>` 下（不是 `hooks.<Event>`），且必须显式 `hooks.enabled: true`（默认禁用）；matcher 是大小写敏感正则，`"*"` 会静默永不匹配，state-only hook 要**省略 matcher**；timeout 字段单位是秒，必须用 `timeoutMs`（毫秒）。ZCode 官方支持 **7 个事件**：SessionStart / UserPromptSubmit / PreToolUse / PermissionRequest / PostToolUse / PostToolUseFailure / Stop（**不支持 SessionEnd / Notification**）。Clawd **Phase 1 只注册 6 个 state-only 事件**（SessionStart / UserPromptSubmit / PreToolUse / PostToolUse / PostToolUseFailure / Stop，不含 PermissionRequest），把状态映射到桌宠动画，hook stdout 恒为 `{}`，**不接管权限、不弹权限气泡**；`PostToolUseFailure → error`（工具失败）。会话结束只能靠 Stop + app 超时回退（无 SessionEnd）。session id 命名空间是 `zcode:<raw>`。远程 SSH 部署不支持 ZCode（与 Qwen 一致，仅 local + WSL）。
