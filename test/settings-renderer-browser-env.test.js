...
const TAB_MODULES = [
  ...
  path.join(SRC_DIR, "settings-tab-ambient.js"),
...
describe("settings renderer browser environment", () => {
  ...
  it("keeps customization visible on Clawd while omitting Calico and removed pets", () => {
  ...
  it("selects an inactive capable user theme and opens its customization in one click", async () => {
    ...
    const testThemeButton = harness.content.querySelectorAll(".theme-customize-btn")
      .find((button) => collectText(findAncestorByClass(button, "theme-card")).includes("Test Theme"));
    assert.ok(testThemeButton);

    testThemeButton.dispatchEvent({ type: "click" });
    assert.deepStrictEqual(
      JSON.parse(JSON.stringify(harness.commands)),
      [{
        name: "setThemeSelection",
        payload: { themeId: "test-theme" },
      }]
    );
    await Promise.resolve();
    await Promise.resolve();
    ...
    assert.ok(collectText(harness.content.querySelector(".theme-detail-heading")).includes("Test Theme"));
