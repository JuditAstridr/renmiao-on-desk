...
test("development launcher uses a configured shared auth API instead of starting a local database", () => {
  assert.equal(resolveRemoteAuthUrl({ RENMI_AUTH_API_URL: "https://auth.renmiao.org/" }), "https://auth.renmiao.org");
  assert.equal(resolveRemoteAuthUrl({ RENMI_AUTH_API_URL: "https://auth.renmiao.org", RENMI_LOCAL_AUTH: "1" }), "");
...
test("development launcher waits for a healthy shared auth API", async () => {
  let calls = 0;
  await waitForRemoteHealth("https://auth.renmiao.org", 100, async () => {
    calls += 1;
    return { ok: calls >= 1 };
  });
