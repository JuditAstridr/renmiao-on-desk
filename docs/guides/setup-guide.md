...
- **Optional debug log:** `CLAWD_QWENWORK_HOOK_DEBUG=1` appends an event/shape summary to `~/.clawd/qwenwork-hook-debug.jsonl` (no prompts, tool inputs or paths). Adding `CLAWD_QWENWORK_HOOK_DEBUG_RAW=1` also stores the complete verbatim payload — **that file then contains sensitive data**; delete it when you are done. Files are created `0600` under a `0700` directory on macOS/Linux.
