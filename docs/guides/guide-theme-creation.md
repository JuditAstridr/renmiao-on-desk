...
- `hitBoxPadding` is optional per-side non-negative motion padding (`left` / `top` / `right` / `bottom`) in the visual's effective viewBox units. Each side is capped to one effective viewBox dimension by schema validation, and runtime hit geometry still clamps the accessory-only contribution to the render-visible viewBox. Use it only for animated `followTarget` motion that extends past `staticFrame`.
